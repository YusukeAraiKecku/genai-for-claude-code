...
このファイルは、Claude Code が `gennai-for-claude-code` リポジトリで作業するときに毎セッション読む最小ルールです。詳細手順は `.claude/skills/` と `specs/` (ある場合) に分離します。

## このプロジェクトは何か

**Gennai for Claude Code** は、AIアプリ仕様 → Claude Code Skill への local-first コンパイラです。
...
- `../gennai_for_claude_code_spec/` (本リポジトリ祖先の仕様書)
