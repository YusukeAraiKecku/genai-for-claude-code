...
| パス | 用途 |
|---|---|
| `examples/` | 公式サンプル Skill。CI (`verify.sh`) の smoke compile/validate/test 対象。 |
| `genai-apps/` | 試用・実験用 recipe の作業ディレクトリ。`.gitignore` 対象、CI 非対象。`examples/` に昇格する前の staging area として使う。 |
| `references/_shared/` | 複数 Skill から参照する共有 rubric・リファレンス。`local_context.references` に `../../references/_shared/...` で指定する。 |
| `packages/core/` | コンパイラ・eval・security チェックのコアロジック。 |
| `packages/cli/` | `genai` CLI エントリポイント。 |
...
- 仕様にない機能を勝手に追加しない。
- 完成前に必ず `bash scripts/verify.sh` を実行する。
- `verify.sh` が失敗する限り、完成と言わない。
...
- 他人の変更を巻き戻さない。
- コミット前に `bash scripts/verify.sh` を実行する。
